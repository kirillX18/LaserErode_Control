"""
step_block.py — переиспользуемые элементы пошагового управления.

Паттерн «строка шага» (подпись + кнопки уменьшения/увеличения) повторяется во
многих блоках: оси XYZ, суставы, параметры лазера. Здесь он вынесен в два
компонента:

    StepRow     — одна строка: [подпись] [−] [+]
    StepControl — группа строк шага с единым колбэком on_step(delta)

Это убирает дублирование и позволяет менять поведение/вид всех шаговых
контролов в одном месте.
"""
