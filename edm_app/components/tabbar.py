"""
tabbar.py — панель вкладок с анимацией наведения.

Штатный QTabBar при наведении ничего не показывает. Здесь добавлена плавная
подсветка под стать кнопкам (которые «приподнимаются» тенью): поверх обычной
отрисовки вкладок (стиль берётся из QSS) рисуется анимированный акцент —
снизу вкладки из центра «вырастает» цветная полоска и слегка проявляется фон.

Прогресс каждой вкладки (0..1) на каждом тике таймера плавно догоняет цель
(1 — курсор над вкладкой, иначе 0), поэтому отдельные объекты-анимации на
каждую вкладку не нужны, а таймер работает только пока есть что анимировать.
Используется и для верхних вкладок, и для внутренних вкладок сервисного
управления — достаточно установить этот класс через setTabBar().
"""
